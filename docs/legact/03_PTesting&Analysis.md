...
During preliminary testing under full load, the Raspberry Pi 5 architecture suffered from severe power delivery failures. Specifically, the robot failed to supply adequate or stable energy to the steering servo due to the Pi's high baseline current draw and sensitivity to vibration-induced connector stress. Because we encountered these critical brownouts and needed a robust, immediate solution to ensure reliable bench performance and drivetrain headroom ahead of competition deadlines, we executed a swift architectural migration to the Arduino Mega 2560. This microcontroller environment entirely eliminated operating system overhead and voltage sags, restoring stable, high-torque power to our actuators.
